...
def test_Entry_class():
    with tempfile.TemporaryDirectory() as dir:
        ...
        assert entry_class.check_field_exists("isbn") == False
        assert entry_class.check_field_exists("abstract") == False
        
        assert entry_class.filter("Contents", utils.QueryFields.ABSTRACT) == False
        assert entry_class.filter("Frontiers", utils.QueryFields.TITLE)
        assert entry_class.filter("Beran, Gregory J. O.", utils.QueryFields.AUTHOR)

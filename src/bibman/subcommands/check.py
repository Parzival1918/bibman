            ...
            for name in files:
                ...
                if name.startswith(".") and name.endswith(
                    ...
                    if not bib_file.is_file():
                        print(
                            f"Note file found that does not have an entry file associated: {name}"
                        )
                    else:
                ...
                if not name.endswith(".bib"):
                    print(
                        f"Found file that is not of .bib extension: {filepath}"
                    )
                ...
                if len(bib_library.entries) > 1:
                    print(
                        f"Found file that contains multiple BibTeX entries: {filepath}"
                    )
